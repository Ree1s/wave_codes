...
import random
from warpflow_utils_ori import *
...
class TokenFlow(nn.Module):
    def __init__(self, config):
        ...
        if sd_version == '2.1':
            ...
            model_key = "stabilityai/stable-diffusion-v1-5"
        ...
        self.paths, self.frames, self.latents, self.eps = self.get_data(self.indice)
        if self.sd_version == 'depth':
            self.depth_maps = self.prepare_depth_maps()
        if True:
            from diffusers.models import ControlNetModel
            controlnet = ControlNetModel.from_pretrained("stabilityai/sd-controlnet-canny", torch_dtype=torch.float16).to(self.device)
    ...
    def get_data(self, indice):
        ...
        eps = self.get_ddim_eps(latents, indice).to(torch.float16).to(self.device)
    ...
    def get_ddim_eps(self, latent, indices):
        ...
        noisy_latent = torch.load(latents_path).to(self.device)[indices]
    ...
    def denoise_step(self, x, t, indices, indices_pivotal):
        ...
        source_latents = load_source_latents_t(t, self.latents_path)[indices_pivotal]
    ...
    def sample_loop_warp_random(self, latents, indices):
        ...
        register_flow(
            self,
            flows = flow_dict['flows_fwd'],
            masks = flow_dict['mask_vis_fwd'],
            size = flow_dict['size'],
            timesteps = len(self.scheduler.timesteps)
        )
...
if __name__ == '__main__':
    ...
    config["output_path"] = os.path.join(config["output_path"] + f'_pnp_SD_{config["sd_version"]}',
                                             ...
                                             config["prompt"][:240],
                                             f'attn_{config["pnp_attn_t"]}_f_{config["pnp_f_t"]}_c_{config["conditioning_scale"]}',
