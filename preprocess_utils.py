        ...
        def forward(self, input_tensor, temb):
            ...
            if self.warp:
                hidden_states_warp = warp_single_latent(
                    ...
                    reference_flow=torch.repeat_interleave(self.flows[[self.batch_idx - 1]], dim=0, repeats=n_frames) 
                )
                mask = torch.repeat_interleave(F.interpolate(self.masks[[self.batch_idx - 1]], size=(h, w)), dim=0, repeats=n_frames)
...
def make_opticalflow_attention_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class OpticalFlowBlock(block_class):
        ...
        def forward(
            ...
            batch_size, sequence_length, dim = hidden_states.shape
            n_frames = batch_size // 1
            hidden_states = hidden_states.view(1, n_frames, sequence_length, dim)
            ...
            norm_hidden_states = norm_hidden_states.view(1, n_frames, sequence_length, dim)
            cross_attention_kwargs = cross_attention_kwargs if cross_attention_kwargs is not None else {}
            ...
            if self.warp:
                ...
                norm_hidden_states_warp = warp_single_latent(
                    latent = rearrange(self.pivotal_hidden_states_list[self.indice].view(n_frames, sequence_length, dim), 'b (h w) c->b c h w', h=self.size[0], w=self.size[1]), 
                    reference_flow = torch.repeat_interleave(self.flows[[self.batch_idx - 1]], dim=0, repeats=n_frames) 
                )
                mask = torch.repeat_interleave(F.interpolate(self.masks[[self.batch_idx - 1]], size=(self.size[0], self.size[1])), dim=0, repeats=n_frames)
                norm_hidden_states_style = mask * norm_hidden_states_warp + (1 - mask) * rearrange(norm_hidden_states.view(n_frames, sequence_length, dim), 'b (h w) c->b c h w', h=self.size[0], w=self.size[1])
                norm_hidden_states_style = rearrange(norm_hidden_states_style, 'b c h w-> b (h w) c').view(1, n_frames, sequence_length, dim)
                norm_hidden_states = norm_hidden_states_style
                concat_hidden_states = torch.cat(
                    [self.pivotal_hidden_states_list[self.indice], norm_hidden_states], dim=2
                )
